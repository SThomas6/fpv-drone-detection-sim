...
for t in data/clips/range_dual; do
...
capture() { # name sdf profile interval seconds hfov seed birds birdseed
  ...
  STATION_HFOV="$hfov" STATION_PITCH="${PITCH:--0.15}" python3 scripts/drive_scene.py --world "$WORLD" \
    --profile "$profile" --seed "$seed" --birds "$birds" --bird-seed "$bseed" \
    --speed-scale 3.0 --accel 15.0 \
  ...
  STATION_HFOV="$hfov" STATION_PITCH="${PITCH:--0.15}" python3 scripts/capture_dataset.py \
    --seconds "$secs" --interval "$interval" --world "$WORLD" --thermal \
    --verify-below-v 250 --zoom "${occ[@]}" --out "data/clips/$name" \
...
export PITCH=-0.2269   # 13 deg, matches tele_sky SDF
capture range_dual      "$W/detection_world_terrain_dual.sdf" skysweep 0.0667 300 1.047 63 4 613
